...
AgentOutput = collections.namedtuple('AgentOutput', 'enum enum_logits x x_logits y y_logits target_unit target_unit_logits ability ability_logits baseline')
...
class Dota(tf.Module):
  ...
  def __init__(self, enum_parametric_action_distribution, x_parametric_action_distribution, 
               ...
               ability_parametric_action_distribution):
    ...
    self.affine_pre_rnn = tf.keras.layers.Dense(128, activation='relu')
    ...
    self.affine_move_y = tf.keras.layers.Dense(self._y_parametric_action_distribution.param_size, name='y_policy_logits',
                                               kernel_initializer='lecun_normal')
    self.affine_unit_attention = tf.keras.layers.Dense(self._target_unit_parametric_action_distribution.param_size, 
                                                       name='target_unit_policy_logits',
                                                       kernel_initializer='lecun_normal')
  ...
  def _torso(self, unused_prev_action, env_output):
    ...
    anh_embedding = self.affine_unit_eh(anh_basic)
    ...
    return unit_embedding, x, enum_mask, x_mask, y_mask, target_unit_mask, ability_mask
  ...
  def _head(self, torso_output):
    ...
    ability_logits_list = []
    for e_l, x_l, y_l, t_l, a_l, e_m, x_m, y_m, t_m, a_m in zip(tf.unstack(action_scores_enum), 
  ...
  def _unroll(self, prev_actions, env_outputs, core_state):
    unused_reward, done, _, _, _, _, _, _, _, _, _, _, _, _, _, _ = env_outputs

    torso_outputs = utils.batch_apply(self._torso, (prev_actions, env_outputs))

    unit_embedding, x, enum_mask, x_mask, y_mask, target_unit_mask, ability_mask = torso_outputs
    ...
    return utils.batch_apply(self._head, ((unit_embedding, core_outputs, enum_mask, x_mask, y_mask, target_unit_mask, ability_mask),)), core_state