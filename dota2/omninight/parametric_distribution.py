...
class TanhTransformedDistribution(tfd.TransformedDistribution):
  ...
  def log_prob(self, event):
    ...
    return tf.where(event <= -self._threshold, self._log_prob_left, tf.where(event >= self._threshold, self._log_prob_right,
...
def normal_clipped_distribution(num_actions, gaussian_std_fn=softplus_default_std_fn):
  ...
  def create_dist(parameters):
    ...
    scale = gaussian_std_fn(scale)
    normal_dist = tfd.Normal(loc=loc, scale=scale)
    return tfd.Independent(ClippedIdentity(normal_dist), reinterpreted_batch_ndims=1)
...
def deterministic_tanh_distribution(num_actions):
  def create_dist(parameters):
    return tfd.Independent(TanhTransformedDistribution(tfd.Deterministic(loc=parameters)), reinterpreted_batch_ndims=1)
...
def continuous_action_config(
  ...
  if action_gaussian_std_fn == 'safe_exp':
    ...
    raise ValueError('Flag `action_gaussian_std_fn` only supports safe_exp and' f' softplus, got: {action_gaussian_std_fn}')
