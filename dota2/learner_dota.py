...
def compute_loss(logger, enum_parametric_action_distribution, 
                 ...
                 agent, agent_state, prev_actions, env_outputs, agent_outputs):
  ...
  rewards, done, _, _, _, _, _, _, _, _, _, _, _, _, _, _ = tf.nest.map_structure(lambda t: t[1:], env_outputs)
...
Action = collections.namedtuple('Action', 'enum_logits x_logits y_logits target_unit_logits ability_logits')
...
def learner_loop():
  ...
  env_output_specs = EnvOutput(
      ...
      tf.TensorSpec([1,12], tf.float32, 'allied_heroes_observation'),
      tf.TensorSpec([5,12], tf.float32, 'enemy_heroes_observation'),
      tf.TensorSpec([16,12], tf.float32, 'allied_nonheroes_observation'),
      tf.TensorSpec([16,12], tf.float32, 'enemy_nonheroes_observation'),
      tf.TensorSpec([1,12], tf.float32, 'allied_towers_observation'),
      tf.TensorSpec([1,12], tf.float32, 'enemy_towers_observation'),
      tf.TensorSpec([4], tf.float32, 'enum_mask'),
      tf.TensorSpec([9], tf.float32, 'x_mask'),
      tf.TensorSpec([9], tf.float32, 'y_mask'),
      tf.TensorSpec([40], tf.float32, 'target_unit_mask'),
    ...
    with tf.device(host):
      ...
      def create_inference_fn(inference_device):
        ...
        def inference(env_ids, run_ids, env_outputs, raw_rewards):
          ...
          return (agent_outputs.enum, agent_outputs.x, agent_outputs.y, agent_outputs.target_unit, agent_outputs.ability)
